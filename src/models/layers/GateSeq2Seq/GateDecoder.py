...
class GateDecoder(nn.Module):
    def __init__(self, num_category, cate_dim, hidden_dim, n_layers, dropout, hidden_type, att):
        ...
        self.att = att
        if self.att == 'T':
            self.fc_out = nn.Linear(hidden_dim * 3, num_category)
        else:
        ...
        if self.att == 'T':
            self.attention = Attention(enc_hid_dim=hidden_dim * 2, dec_hid_dim=hidden_dim)
